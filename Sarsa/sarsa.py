...
class Sarsa():
    ...
    def episode(self,env = None):
        '''
        Implementation of Sarsa algorithm based on pseudo code from Chapter 6 of "Reinforcement Learning: An Introduction"
        ...
        while not terminated:
            ...
            self.Q[s,action] = self.Q[s,action] + self.stepsize * (reward + (self.gamma * self.Q[s_prime,action_prime]) - self.Q[s,action])
            s = s_prime
            action = action_prime
            time_steps += 1
...
if __name__ == "__main__":
    ...
    sarsa.train(5000)
    env.close()
    sarsa.watch("FrozenLake-v1",is_slippery = True)
